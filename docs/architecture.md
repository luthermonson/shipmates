...
9. **Claude Code hook surface.** **PARTLY RESOLVED (verified June 2026).** `type: "http"` hooks are real and work: a `--settings` config with `PreToolUse`/`PostToolUse` HTTP hooks fired in `-p` stream-json mode and POSTed `{tool_name, ...}` to the shipmates server's `/hook/<persona>/<event>` endpoint, which returned `200 {}` to let the tool proceed. Tool activity now reaches the feed end-to-end (`tell → PreToolUse:Glob → PostToolUse:Glob → assistant → result`). Still unverified: (a) `PermissionRequest` returning `{behavior:"allow"|"deny"}` to gate a tool, (b) `SessionStart`/`SessionEnd` firing as expected, (c) the full ~30-event list. Next hook to prove is `PermissionRequest` (the captain-approval path).
