...
// AdvisoryModel is the single pinned model identity an auto-captain advisory
// turn may claim. The value is inherited from the reference journal contract
// (the closed feat/runtime-interface branch) so persisted assessment records
// keep one stable schema; it is a schema constant, not a claim that this
// model is dispatchable on the current runtime.
const AdvisoryModel = "gpt-5.6-sol"
...
func (r ResponseV1) Validate() error {
	...
	if r.PersonaDigest != "" && (!hashPattern.MatchString(r.PersonaDigest) || r.PersonaVersion == 0 || r.PersonaVersion > 8 || r.EffectiveModel != AdvisoryModel) {
...
func OpenJournal(path string) (*Journal, error) {
	...
	if err == nil && (info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || journalWorldAccessible(info) || info.Size() > MaxJournalBytes) {
