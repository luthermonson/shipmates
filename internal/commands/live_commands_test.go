...
func writeLiveCommandDiscovery(t *testing.T) {
	t.Helper()
	// The server discovery record is normally written via
	// project.WriteServerStateFile, which enforces owner+SYSTEM-only DACLs on
	// Windows (see server_state_windows.go). On unix the same helper wraps
	// flock+fsync. The Windows DACL round-trip now verifies, so this runs
	// everywhere.
