...
// alive in every project dir listed in ~/.shipmates/ship.yaml. On unix it also
// carries the Fleet Commander `observe` subcommand; see shipcmd_other.go for
// the platforms where only the supervisor exists.
...
func shipWithProductionReadiness(interruptClock fleetsteer.Clock, ready chan<- struct{}) *cli.Command {
	return shipCommand([]*cli.Command{shipObserveCommand(interruptClock, ready)})
}

// shipObserveCommand publishes this project's read-only state through the
// authenticated Fleet tunnel. Unix-only: fleettunnel and fleetsteer are
// built on openat/O_NOFOLLOW/flock.
func shipObserveCommand(interruptClock fleetsteer.Clock, ready chan<- struct{}) *cli.Command {
	return &cli.Command{
