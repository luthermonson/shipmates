...
// Binary resolves the real external bd CLI, skipping the calling test when bd
// is not installed unless RequiredEnv is set to 1.
func Binary(t testing.TB) string {
	...
	t.Skipf("real bd CLI not found on PATH (%v); install bd %s or set %s to run the external Beads contract tests", lookErr, Version, BinaryEnv)
	return ""
}
