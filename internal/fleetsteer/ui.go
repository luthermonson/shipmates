...
// steerJS owns one operation identifier per steer. It is minted before the
// first attempt and deliberately retained while the outcome is unknown, so
// re-affirming and submitting again replays that exact operation instead of
// steering the same live turn a second time. A decided outcome mints a new one.
const steerJS = `(()=>{'use strict';const q=x=>document.getElementById(x),f=q('steer'),m=q('message'),b=q('bytes'),s=q('submit'),o=q('result'),a=q('approval'),x=q('selected');let pending=false,opid='';const newOperationID=()=>{const u=new Uint8Array(32);crypto.getRandomValues(u);return btoa(String.fromCharCode.apply(null,u)).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'')};const bytes=()=>new TextEncoder().encode(m.value).length,enable=()=>{b.textContent=bytes()+' / 4096 bytes';s.disabled=pending||a.checked||!x.checked||bytes()<1||bytes()>4096};m.addEventListener('input',enable);a.addEventListener('change',enable);x.addEventListener('change',enable);enable();f.addEventListener('keydown',e=>{if(e.key==='Enter'&&e.target.tagName!=='TEXTAREA')e.preventDefault()});f.addEventListener('submit',async e=>{e.preventDefault();if(pending||a.checked||!x.checked||bytes()<1||bytes()>4096)return;pending=true;enable();o.textContent='submitted; waiting for the exact-turn decision';const credential=q('credential').value;q('credential').value='';if(!opid)opid=newOperationID();const body={schema_version:1,fleet_id:q('fleet').value,fleet_epoch:Number(q('epoch').value),ship_id:q('ship').value,connection_generation:Number(q('generation').value),persona:q('persona').value,steer_target_ref:q('target').value,message:m.value,operation_id:opid};try{const r=await fetch('/api/fleet/v1/turn-steers',{method:'POST',headers:{Authorization:'Bearer '+credential,'Content-Type':'application/json'},body:JSON.stringify(body),cache:'no-store',credentials:'omit',referrerPolicy:'no-referrer'}),v=await r.json();if(v.outcome==='accepted'){opid='';o.textContent='steer accepted for this turn; effect not yet known'}else if(v.outcome==='refused'){opid='';o.textContent='steer refused: '+String(v.reason_code).slice(0,64)}else o.textContent='delivery outcome unknown; affirm and submit again to replay this same operation'}catch(_){o.textContent='delivery outcome unknown; affirm and submit again to replay this same operation'}finally{pending=false;x.checked=false;enable()}})})();`
